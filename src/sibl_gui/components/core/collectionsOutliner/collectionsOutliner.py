			...
			if name != self.__overallCollection:
				if not self.collectionExists(name):
					...
					if self.addCollection(name, comment):
						self.__view.selectionModel().setCurrentIndex(self.__model.getNodeIndex(self.__model.findChildren(r"\b{0}\b".format(name))[0]), QItemSelectionModel.Current | QItemSelectionModel.Select | QItemSelectionModel.Rows)
						return name
					else:
