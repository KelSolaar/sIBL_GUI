...
class Inspector(UiComponent):
	...
	def __Inspector_DockWidget_setUi(self):
		...
		if self.__inspectorIblSet:
			...
			if self.__inspectorIblSet.previewImage:
				self.ui.Image_label.setPixmap(siblgui.ui.common.getPixmap(self.__inspectorIblSet.previewImage))
				self.__drawInspectorIblSetOverlay()
			else:
				self.ui.Image_label.setText(self.__noPreviewImageText.format(siblgui.ui.common.filterImagePath(self.__inspectorIblSet.icon), self.__inspectorIblSet.author, self.__inspectorIblSet.link))
	...
	def __Inspector_DockWidget_clearUi(self):
		...
		self.ui.Image_label.setText(self.__noInspectorIblSetText.format(siblgui.ui.common.filterImagePath("")))
	...
	def __Plates_listView_setModel(self):
		...
		if self.__inspectorIblSet:
			...
			inspectorIblSetStandardItem.setIcon(siblgui.ui.common.getIcon(self.__inspectorIblSet.icon))
			...
			for name, plate in self.__inspectorPlates.items():
				...
				try:
					...
					plateStandardItem.setIcon(siblgui.ui.common.getIcon(plate.icon))
	...
	def __Plates_listView_selectionModel__selectionChanged(self, selectedItems, deselectedItems):
		...
		if item:
			if hasattr(item, "_datas"):
				self.ui.Image_label.setPixmap(siblgui.ui.common.getPixmap(item._datas.previewImage))
			else:
	...
	@core.executionTrace
	@foundations.exceptions.exceptionsHandler(siblgui.ui.common.uiBasicExceptionHandler, False, Exception)
